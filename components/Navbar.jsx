...
const Navbar = () => {
    ...
    return (
        <>
            ...
            <nav className={`w-full fixed px-5 lg:px-8 xl:px:[8%] py-4 flex items-center justify-between bg-white z-50 ${isScrolled ? 'bg-white bg-opacity-50 backdrop-blur-lg shadow-sm' : ''}`}>
                <a href='#top'>
                    <Image src={assets.seid} alt='logo' className="w-28 cursor-pointer mr-14" />
                </a>
                <ul className={`hidden md:flex items-center gap-6 lg:gap-10 xl:gap-14 text-sm lg:text-base xl:text-lg font-semibold text-gray-700 py-3 px-12 rounded-full ${isScrolled ? '' : 'bg-white bg-opacity-50 shadow-sm'}`}>
                ...
                <ul ref={sideMenuRef} className='flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500'>
                    <div className='absolute top-6 right-6'>
                        <Image src={assets.close_black} alt='close icon' onClick={closeMenu} className='w-5 cursor-pointer' />
                    </div>
